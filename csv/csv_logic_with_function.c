...
static int ends_with_carriage_return(const char* p, size_t size)
{
    return size >= 2 && p[-1] == '\r';
}
...
static void CsvTerminateLine(char* p, size_t size)
{
     ...
     */
    char* res = p;
    if (ends_with_carriage_return(res, size))
        --res;

    *res = 0;
}

static int is_newline_outside_quotes(char c, CsvHandle handle)
{
    return c == '\n' && !(handle->quotes & 1);
}
...
char* handle_quote_and_newline(char c, int n, char quote, CsvHandle handle, char* p) {
    if (c == quote) {
        handle->quotes++;
    } else if (is_newline_outside_quotes(c, handle)) {
        return p + n;
    }
}
...
static int is_escape_sequence(const char* p, CsvHandle handle)
{
    return *p == handle->escape && p[1];
}

static int is_double_quote(const char* p, CsvHandle handle)
{
    return *p == handle->quote && p[1] == handle->quote;
}

static int should_end_quoted_field(int quoted, int dq, const char* p, CsvHandle handle)
{
    return quoted && !dq;
}
...
const char* CsvReadNextCol(char* row, CsvHandle handle)
{
    ...
    for (; *p; p++, d++)
    {
        ...
        /* skip escape */
        if (is_escape_sequence(p, handle))
            p++;

        /* skip double-quote */
        if (is_double_quote(p, handle))
        {
            dq = 1;
            p++;
        }
        ...
        if (should_end_quoted_field(quoted, dq, p, handle))
        {
            if (*p == handle->quote)
                break;
        }
