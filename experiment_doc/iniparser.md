...
tolower
```
大文字のアルファベットを小文字に変換します。引数が大文字でない場合は、何もせず指定された値を返します。
...
strlen
```
文字列の長さを計算します。文字列の先頭から、終端文字`\0`の直前までの文字数を返します。
...
malloc
```
指定されたバイトサイズのメモリブロックをヒープ領域に確保します。\
確保したメモリ両機の先頭アドレスを返しますが、確保に失敗した場合は、NULLを返します。
...
memcpy
```
指定されたバイト数だけ、あるメモリ領域から別のメモリ領域へデータをコピーします。
...
isspace
```
指定された文字が空白文字かどうかを判定します。\
空白文字であれば0以外の値、そうでなければ0を返します。
...
memmove
```
指定されたバイト数だけ、あるメモリ領域から別のメモリ領域へデータを移動させます。
...
vfprintf
```
va_startで初期化された可変長引数リストを受け取って、ファイルに書式付きで出力します。
...
strtoimax
```
文字列を、実行している環境で最大の符号付き整数型に変換します。
...
strtoumax
```
文字列を、実行している環境で最大の符号なし整数型に変換します。
...
feof
```
指定されたファイルストリームがファイルの終端に達しているかを判定します。終端に達していれば0以外の値を返します。
...
fopen
```
ファイルを開き、そのファイルにアクセスするためのストリームを返します。
